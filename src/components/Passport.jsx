...
import {
  ...
  SiHtml5, SiCss3, SiJavascript, SiDocker, SiAmazonwebservices, SiGit, SiFastapi
} from 'react-icons/si';
import { FaDatabase, FaBrain, FaRobot, FaTable, FaServer } from 'react-icons/fa';
...
const TechTag = ({ name, color = 'bg-wc-blue' }) => {
  const getSkillIcon = (skill) => {
    ...
    if (s.includes('javascript') || s.includes('js')) return <SiJavascript className="text-[#F7DF1E]" />;
    if (s.includes('docker')) return <SiDocker className="text-[#2496ED]" />;
    if (s.includes('aws')) return <SiAmazonwebservices className="text-[#232F3E]" />;
    if (s.includes('git')) return <SiGit className="text-[#F05032]" />;
    if (s.includes('fastapi')) return <SiFastapi className="text-[#009688]" />;
  ...
  const getSkillColor = (skill) => {
    const s = skill.toLowerCase();
    if (s.includes('html') || s.includes('css') || s.includes('javascript') || s.includes('js')) return 'bg-wc-yellow';
    ...
    if (s.includes('docker') || s.includes('aws') || s.includes('git') || s.includes('fastapi')) return 'bg-wc-blue';
    return 'bg-wc-blue';
  };
...
const pages = [
  ...
  {
    title: "Visa: First Code",
    type: "Entry Permit",
    content: (
      <div className="relative h-full flex flex-col justify-center">
        <div className="absolute -top-4 -right-4 w-24 h-24 border-4 border-wc-yellow/30 rounded-full flex items-center justify-center -rotate-12">
          <span className="text-wc-yellow font-black text-xs text-center leading-tight">ENTRY<br />GRANTED<br />2022</span>
        </div>
        <div className="space-y-4">
          <h4 className="text-xl md:text-2xl font-heading font-black text-ink/80">The Beginning</h4>
          <p className="text-base text-ink/60 font-sans italic">"Where the journey started. Writing my first lines of code and building the foundation of web and logic."</p>
    ...
    content: (
      <div className="relative h-full flex flex-col justify-center">
        <div className="absolute -top-8 -right-4 w-24 h-24 border-4 border-wc-rose/30 flex items-center justify-center rotate-12">
          <span className="text-wc-rose font-black text-xs text-center leading-tight">BRAIN<br />MAPPED<br />2024</span>
        </div>
        <div className="space-y-4">
          <img src={magicBus} alt="Profile" className="w-16 md:w-20 opacity-50 grayscale mx-auto" />
          <h4 className="text-xl md:text-2xl font-heading font-black text-ink/80">AI & Data</h4>
          <p className="text-base text-ink/60 font-sans italic">"Teaching machines to see and understand. Exploring the world of data and neural networks."</p>
  ...
  {
    title: "Visa: Production",
    type: "Industry Verified",
    content: (
      <div className="relative h-full flex flex-col justify-center">
        <div className="absolute -top-4 -right-4 w-24 h-24 border-4 border-wc-blue/40 flex items-center justify-center -rotate-6">
          <span className="text-wc-blue font-black text-xs text-center leading-tight tracking-widest">DEPLOYED<br />IN<br />WILD</span>
        </div>
        <div className="space-y-4">
          <h4 className="text-xl md:text-2xl font-heading font-black text-ink/80">MLOps & Deployment</h4>
          <p className="text-base text-ink/60 font-sans italic">"Taking models out of the lab and into the real world. Building systems that run smoothly at scale."</p>
          <div className="flex flex-wrap pt-2">
            <TechTag name="Docker" />
            <TechTag name="AWS" />
            <TechTag name="FastAPI" />
            <TechTag name="Git" />
          </div>
        </div>
      </div>
